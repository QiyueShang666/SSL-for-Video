...
from Utils.utils import AverageMeter, ConfusionMeter, save_checkpoint, write_log, calc_topk_accuracy, denorm, calc_accuracy
...
parser.add_argument('--test1', default='', type=str)
...
def main():
    ...
    global cuda; cuda = torch.device('cuda:0')
    ...
    if args.train_what == 'ft':
        ...
        for name, param in model.module.named_parameters():
            if ('backbone' in name) or ('transEnc' in name):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # 保存embedding
        if (epoch + 1) % 10 == 1:
            print(f'>>> Epoch {epoch+1}: extracting and saving embeddings ...')
            model.eval()
            all_embs, all_labels = [], []
            with torch.no_grad():
                for input_seq, target in val_loader:
                    input_seq = input_seq.to(cuda)
                    # 提取 [B, D] 维度的 embedding
                    emb = model.module.forward_embedding(input_seq)
                    all_embs.append(emb.cpu())
                    all_labels.append(target.view(-1).cpu())
            embeddings = torch.cat(all_embs, dim=0)  # [num_samples, D]
            labels     = torch.cat(all_labels, dim=0)  # [num_samples]
            save_path = os.path.join(model_path,
                                     f'embeddings_epoch{epoch+1}.pt')
...
def train(data_loader, model, optimizer, epoch):
    ...
    model.train()
    global iteration, val_loader, model_path, cuda, writer_val
    ...
    for idx, (input_seq, target) in enumerate(data_loader):
        ...
        if idx % args.print_freq == 0:
            ...
            decay_weight = 0.0
            for name, param in model.module.named_parameters():
                if 'transEnc' in name and param.requires_grad:
                    decay_weight += param.norm(2).item()
                total_weight += param.norm(2).item()
            print(f'Decay weight / Total weight: {decay_weight:.3f}/{total_weight:.3f}')
...
def test(data_loader, model):
    ...
    with torch.no_grad():
        for idx, (input_seq, target) in tqdm(enumerate(data_loader), total=len(data_loader)):
            ...
            output = model(input_seq)
            if target.squeeze(0) == torch.argmax(torch.mean(
                                        torch.mean(
...
lr{1}_wd{args.wd}_ds{args.ds}_seq{args.num_seq}_\
