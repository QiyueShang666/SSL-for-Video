...
class LC(nn.Module):
    def __init__(self, sample_size, num_seq, network='resnet18', dropout=0.6, num_class=101):
        ...
        self.param['num_layers'] = 2  # param for Transformer
        self.param['num_heads'] = 4  # param for Transformer
    ...
    def forward_embedding(self, block):
        """
        提取 ConvGRU 聚合后的上下文表示（不经过分类头），
        返回形状 [B, D] 的 embedding。
        """
        B, N, C, H, W = block.shape
        block = block.view(B * N, C, H, W)
        feature = self.backbone(block)            # [B*N, D, 4,4]
        feature = F.relu(feature)
        feature = feature.view(B, N, self.param['feature_size'], self.last_size, self.last_size)

        # Transformer
        tokens = feature.permute(0,1,3,4,2).contiguous().view(B, -1, self.param['feature_size'])
        seq = self.transEnc(tokens)
        seq = seq.view(B, N, self.last_size, self.last_size, self.param['feature_size']) \
                 .permute(0,1,4,2,3).contiguous()
        seq = F.relu(seq)

        # ConvGRU 聚合
        context, _ = self.agg(seq)               # [B, N, D]
        emb = context[:, -1, :]                  # 取最后时刻 [B, D]
        return emb
